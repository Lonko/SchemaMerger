...
public class RationalCurveFunction implements CurveFunction {
    ...
    /*
     * the values of f(x) for each integer 1 <= x <= x0 each value y' is stored
     * in the element in the x'-1 position (with x' being the x such that f(x')
     * = y')
     */
    private int[] yValues;
    ...
    /*
     * calculates f(x) (as an int) for each x in [1,x0] for curves with a known
     * x0
     */
    ...
    /*
     * calculates f(x) (as an int) for each x in [1,x0] for curves without a
     * known x0
     */
