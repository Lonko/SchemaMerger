...
public class MongoSyntheticDao implements SyntheticDatasetDao {
	...
	public void uploadCatalogue(List<CatalogueProductPage> catalogue, boolean delete) {
		...
		while (uploadedProds != catalogue.size()) {
			int size = (catalogue.size() - uploadedProds > BATCH_SIZE) ? BATCH_SIZE : catalogue.size() - uploadedProds;
	...
	public void uploadSource(List<SourceProductPage> productPages) {
		...
		while (uploadedProds != productPages.size()) {
			int size = (productPages.size() - uploadedProds > BATCH_SIZE) ? BATCH_SIZE
	...
	public void finalizeSourceUpload() {
		MongoCollection<Document> productCollection = this.database
		...
		if (schemaCollection.count() == 0) {
			...
			products.find()
					.projection(Projections.fields(
							Projections.include(MongoDbUtils.SPECS, MongoDbUtils.WEBSITE, MongoDbUtils.CATEGORY)))
	...
	private MongoCollection<Document> getCollectionWithWriteConcern(String collectionName) {
		return this.database.getCollection(collectionName).withWriteConcern(WriteConcern.JOURNALED);
	}
	...
	private Document schema2Document(Map.Entry<String, Set<String>> sourceSchema) {
		...
		return new Document(MongoDbUtils.CATEGORY, category).append(MongoDbUtils.WEBSITE, website)
				.append(MongoDbUtils.ATTRIBUTES, sourceSchema.getValue());
	}
