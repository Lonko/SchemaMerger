...
public class MongoDBConnector {
	...
	public List<Document> getRLSample(int size, String category){
		...
		collection.aggregate(Arrays.asList(matchBson, sampleBson))
				  .forEach((Document d) -> sample.add(d));
	...
	public Map<Document, List<Document>> getProdsInRL(List<String> websites, String category){
		...
		//get all product's pages in "catalog" with linkage (unwinding the linkage attribute)
		List<Document> prods = new ArrayList<>();
	...
	public Document getIfValid(String url){
		return this.database.getCollection("Products")
							.find(Filters.and(Filters.eq("url",url), 
								  Filters.ne("linkage", Collections.EMPTY_LIST),
								  Filters.ne("spec", new Document())))
							.first();
	}
	
	public String getWebsite(String url){
		String website;
		Document doc = this.database.getCollection("Products")
									 .find(Filters.eq("url", url))
									 .first();
		try{
	...
	private void initializeProducts(MongoCollection<Document> collection){
		...
		websites.entrySet().stream()
						   .map(this::website2Documents)			   
	...
	private void initializeRecordLinkage(MongoCollection<Document> collection){
		...
		@SuppressWarnings("unchecked")
		Collection<JSONObject> jsonCollection = Collections.checkedCollection(
													this.fdc.readRecordLinkage().values(), JSONObject.class);
		jsonList.addAll(jsonCollection);
		jsonList.stream()
				.map(this::urls2Documents)
				.filter(list -> list.size() > 0)
				.forEach(collection::insertMany);
	...
	/* should only be called after initializeProducts has already been called once */
	@SuppressWarnings("unused")
	private void initializeSchemas(MongoCollection<Document> collection){
		...
		products.find()
				.projection(Projections.fields(Projections.include("spec", "website", "category")))      
				.forEach((Document d) -> loadSchema(d, schemas));
        
		schemas.entrySet().stream()
						  .map(this::schema2Document)
	...
	/* should only be called after both Products and RecordLinkage have been initialized */
	private void updateProductsRL(){
		...
		try {
		    while (cursor.hasNext()) {
				...
				updates.add(new UpdateOneModel<Document>(new Document("url", u1),
                        						 new Document("$push", new Document("linkage", u2))));
				updates.add(new UpdateOneModel<Document>(new Document("url", u2),
						  						 new Document("$push", new Document("linkage", u1))));
		    }
		} finally {
		    cursor.close();
		}		
	...
	private Document schema2Document(Map.Entry<String, Set<String>> sourceSchema){
		...
		return new Document("category", category)
			   .append("website", website)
			   .append("attributes", sourceSchema.getValue());
	}
	...
	private List<Document> website2Documents(Map.Entry<String, String> website){
		...
		for(File source : sources){
			for(Object json : this.fdc.readSource(source.getAbsolutePath())){
				...
				doc.append("linkage", new ArrayList<>())
				   .append("website", website.getKey())
	...
	private String getValidJSON(JSONObject json){
		...
		List<String> invalidFields = fields.stream()
			.filter(field -> field.contains(".") || field.startsWith("$"))
			...
			for(int i = 0; i < urls.size()-1; i++)
				for(int j = i+1; j < urls.size(); j++){
					Document doc = new Document("category", (String) cat)
								   .append("url1", (String) urls.get(i))
					   			   .append("url2", (String) urls.get(j));
					docs.add(doc);
				}
		}
