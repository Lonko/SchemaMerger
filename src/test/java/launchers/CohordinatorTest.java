...
public class CohordinatorTest {
	...
	public void testEvaluateSyntheticInWrongCluster() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1,1,2), Arrays.asList(2)), 0.333, 0.5);
	}
	
	@Test
	public void testEvaluateSyntheticSplitOneCluster() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1,1), Arrays.asList(2), Arrays.asList(2)), 1, 0.5);
	}
	
	@Test
	public void testEvaluateSyntheticSplitOneBigCluster() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1,1), Arrays.asList(2,2,2), Arrays.asList(2,2)), 1, 5./11);
	}
	
	@Test
	public void testEvaluateSyntheticMerge2Clusters() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1,1,2,2), Arrays.asList(3)), 0.333, 1);
	}
	
	@Test
	public void testEvaluateSyntheticGroupsCorrect() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1,1,1), Arrays.asList(2,2,2)), 1, 1);
	}
	...
	public void testEvaluateSyntheticSingleGroupWrong() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1), Arrays.asList(1), Arrays.asList(1), Arrays.asList(1)), 1, 0);
	}		
	
	@Test
	public void testEvaluateSyntheticAllSeparatedCorrect() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1), Arrays.asList(2),Arrays.asList(3), Arrays.asList(4)), 1, 1);
	}	
	
	@Test
	public void testEvaluateSyntheticAllSeparatedWrong() {
		evaluateSyntheticResultsHelper(Arrays.asList(
				Arrays.asList(1,2,3,4)), 0, 1);
	}	
	
	/**
	 * Example: {{1,1,2}, {2}} --> indicates that there are 2 clusters, and one of the elements are in the wrong one.
	 * @param input 
	 */
